...
//	private static long keepAliveTime = 0L;
	public static ServerStart getInstance(){
        InputStream inStream = ServerStart.class.getClassLoader().getResourceAsStream("sdocker.properties");
        Properties properties = new Properties();
        try {
            properties.load(inStream);
            coreSize = properties.getProperty("threadCoreSize");
            maximumPoolSize = properties.getProperty("threadMaximumPoolSize");
            keepAliveTime = properties.getProperty("threadKeepAliveTime");
            queueCapacity = properties.getProperty("threadQueueCapacity");
        }catch (IOException e){
            e.printStackTrace();
        }finally {
            try {
