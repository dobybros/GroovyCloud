...
import com.dobybros.chat.rpc.RPCClientAdapter.ClientAdapterStatusListener;
import com.dobybros.chat.rpc.impl.ExpireListener;
...
public class RPCClientAdapterMapTask extends Task {
	...
	public synchronized RPCClientAdapter registerServer(String ip, int rmiPort, final String serverName, ClientAdapterStatusListener statusListener) {
		return registerServer(ip, rmiPort, serverName, statusListener, expireTime);
	}
	public synchronized RPCClientAdapter registerServer(String ip, int rmiPort, final String serverName, ClientAdapterStatusListener statusListener, Long expireTime) {
		if(serverName == null)
			return null;
		RPCClientAdapter clientAdapter = clientAdapterMap.get(serverName);
		if (clientAdapter != null) {
			RMIHandler rmiHandler = (RMIHandler) clientAdapter;
		...
		if(clientAdapter == null) {
			if(ip == null) 
				return null; 
			RMIHandler rmiClient = new RMIHandler();
