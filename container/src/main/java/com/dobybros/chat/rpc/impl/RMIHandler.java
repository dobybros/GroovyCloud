...
import com.dobybros.chat.constants.BalancerConstants;
...
import com.dobybros.chat.utils.AverageCounter;
...
public class RMIHandler extends RPCClientAdapter {
...
//    @Resource
    ...
    public String toString() {
        StringBuilder builder = new StringBuilder(RMIHandler.class.getSimpleName() + ": ");
        ...
        public ClientMonitorThread(boolean value) {
            ...
            if(connected.get()) {
                for(ClientAdapterStatusListener statusListener : statusListeners) {
                    try {
                        statusListener.connected(serverName);
                    } catch (Throwable t) {
            ...
            while(isStarted) {
                if(expireTime != null && expireListener != null) {
                    if(touch + expireTime < System.currentTimeMillis()) {
                        try {
                            if (expireListener.expired(RMIHandler.this, touch, expireTime)) {
                                break;
                            }
                        } catch (Throwable t) {
                ...
                if(isStarted) {
                    LoggerEx.info(TAG, "RMI " + serverHost +  " port " + rmiPort + " server " + serverName + " start connecting...");
                    try {
...
//                        serverImpl.initClient(server);
                        ...
                        LoggerEx.info(TAG, "RMI " + serverHost +  " port " + rmiPort + " server " + serverName + " connected!");
                    } catch(Throwable t) {
                        t.printStackTrace();
                        if(t instanceof  NotBoundException) {
                            LoggerEx.error(TAG, "RMI " + serverHost +  " port " + rmiPort + " server " + serverName + " not bound any more, " + t.getMessage());
                        } else {
                            LoggerEx.error(TAG, "RMI " + serverHost +  " port " + rmiPort + " server " + serverName + " connect failed, " + t.getMessage());
                        }
                        try {
                            Thread.sleep(5000L);
                        } catch (InterruptedException e) {
...
//                }
            ...
            if(server != null) {
                try {
                    ...
                    LoggerEx.info(TAG, "RMI " + serverHost +  " port " + rmiPort + " server " + serverName + " is destroyed, unexport " + server + " result " + bool);
                } catch (NoSuchObjectException e) {
//             e.printStackTrace();
                    LoggerEx.info(TAG, "RMI " + serverHost +  " port " + rmiPort + " server " + serverName + " is destroyed, NoSuchObject " + server + " error " + e.getMessage());
                }
            }
            LoggerEx.info(TAG, "RMI " + serverHost +  " port " + rmiPort + " server " + serverName + " monitor stopped...");
            for(ClientAdapterStatusListener statusListener : statusListeners) {
                try {
                    statusListener.terminated(serverName);
                } catch (Throwable t) {
    ...
    public synchronized void serverStart(String ip) {
        ...
        if(rmiId.equals(BalancerConstants.PENDING_SERVER)) {
            return;
        }
        try {
...
//            server = serverImpl.initServer();
            ...
            t.printStackTrace();
            LoggerEx.fatal(TAG, "RMIHandler server start failed. Server will be shutdown... " + t.getMessage());
//            System.exit(1);
        } finally {
            resetSslProp();
        }
    ...
    public synchronized void clientStart() {
        ...
        for(ClientAdapterStatusListener statusListener : statusListeners) {
            try {
                statusListener.started(serverName);
            } catch (Throwable t) {
        ...
        try {
            if(StringUtils.isBlank(serverHost))
                throw new CoreException(CoreErrorCodes.ERROR_ILLEGAL_PARAMETER, "Server host is illegal, " + serverHost);
            if (enableSsl) {
                setSslProp();
...
//       System.out.println(new String(server.call("world".getBytes(), "ab1c", 1)));
            LoggerEx.info(TAG, "RMI " + serverHost +  " port " + rmiPort + " server " + serverName + " client connected!");
            startConnected = true;
        } catch(Throwable t) {
            t.printStackTrace();
...
//       throw new CoreException(CoreErrorCodes.ERROR_UNKNOWN, "Initialize RMIHandler " + serverHost + " | " + rmiPort + " faield, " + t.getMessage());
        } finally {
            touch();
...
//        System.setProperty("javax.net.ssl.keyStore", "/Users/liyazhou/workspace/tcpssl/certificate/server.jks");
    ...
    public synchronized void serverDestroy() {
        try {
            ...
            LoggerEx.info(TAG, "RMI " + serverHost +  " port " + rmiPort + " server " + serverName + " server is destroyed!");
        } catch (Throwable e) {
            e.printStackTrace();
            LoggerEx.error(TAG, "RMI " + serverHost +  " port " + rmiPort + " server " + serverName + " server destroy failed, " + e.getMessage());
        }
    }
    ...
    public RPCResponse call(RPCRequest request) throws CoreException {
        try {
            ...
            for(ClientAdapterStatusListener statusListener : statusListeners) {
                try {
                    statusListener.called(serverName, request, response);
                } catch (Throwable t) {
            ...
            for(ClientAdapterStatusListener statusListener : statusListeners) {
                try {
                    statusListener.callFailed(serverName, request, e);
                } catch (Throwable t) {
    ...
    private RPCResponse callPrivate(RPCRequest request) throws CoreException {
        ...
        if(!clientMonitorThread.connected.get())
            throw new CoreException(CoreErrorCodes.ERROR_RPC_DISCONNECTED, "RPC (" + serverHost + ":" + rmiPort + ") is disconnected for " + request.getType() + ": " + request.toString());
        try {
            byte[] requestData = request.getData();
            String requestType = request.getType();
            if(requestType == null)
                throw new CoreException(CoreErrorCodes.ERROR_RPC_REQUESTTYPE_ILLEGAL, "RPCRequest type is null");
            if(requestData == null) {
                ...
                try {
                    ...
                    if(request.getData() == null)
                        throw new CoreException(CoreErrorCodes.ERROR_RPC_REQUESTDATA_NULL, "RPCRequest data is still null");
                } catch (Throwable t) {
                    throw new CoreException(CoreErrorCodes.ERROR_RPC_PERSISTENT_FAILED, "Persistent RPCRequest " + request.getType() + " failed " + t.getMessage());
                }
            }
            ...
            try {
                ...
                throw new CoreException(CoreErrorCodes.ERROR_RPC_RESURRECT_FAILED, "RPCResponse " + requestType + " resurrect failed, " + t.getMessage());
            }
            ...
            throw new CoreException(CoreErrorCodes.ERROR_RMICALL_CONNECT_FAILED, "RMI call failed, " + ce.getMessage() + " start reconnecting...");
        } catch (Throwable t) {
            ...
            if(t instanceof CoreException)
                throw (CoreException) t;
            t.printStackTrace();
            throw new CoreException(CoreErrorCodes.ERROR_RMICALL_FAILED, "RMI call failed, " + t.getMessage());
        }
    }
    ...
    RPCEntity getRPCEntityForClient(String requestType, RPCRequest request) throws CoreException {
        ...
        if(entity == null) {
            ...
            if(!requestClass.endsWith(REQUEST_SUFIX))
                throw new CoreException(CoreErrorCodes.ERROR_RPC_ILLEGAL, "RequestClass " + requestClass + " don't contain Request as sufix. ");
            ...
            try {
                ...
                e.printStackTrace();
                throw new CoreException(CoreErrorCodes.ERROR_RPC_TYPE_REQUEST_NOMAPPING, "RPC type " + requestType + " don't have correct class name " + responseClassString + ". " + e.getMessage());
            }
...
//    String serverHost = "192.168.122.229";
//    int rmiPort = 9838;
//    RMIHandler handler = new RMIHandler();
////      handler.serverStart();
//    Registry registry = LocateRegistry.getRegistry(serverHost, rmiPort);
//    RMIServer server = (RMIServer) registry.lookup(RMI_ID + RMIID_SSL_SUFFIX);
//
//    System.out.println(server.alive());
        ...
        array.add(1);
//        array.add(userInfo);
        String arrayStr = array.toJSONString();

        List<Object> objs = JSON.parseArray(arrayStr, new Type[]{String.class, Object.class, UserInfo.class});
        System.out.print("");
    }
