...
public abstract class OnlineUserManager {
	...
	@Resource
	protected OnlineServerWithStatusWithRPC onlineServer;
	@Resource
...
//	private int inactiveCheckPeriod = 60;
	private long inactiveCheckPeriod = TimeUnit.SECONDS.toMillis(300);
	private long maxInactiveInterval = TimeUnit.SECONDS.toMillis(1200);
			...
			if(onlineUserHolder != null) {
				for(OnlineUser user : onlineUserHolder.onlineUsers()) {
					...
					for(OnlineServiceUser serviceUser : values) {
						Long touch = serviceUser.getTouch();
						if(touch == null)
							touch = 0L;
						Long maxTime = serviceUser.getMaxInactiveInterval();
						if(maxTime == null) {
							maxTime = maxInactiveInterval;
						}
						if(/*!user.keepOnline() && */System.currentTimeMillis() - touch > maxTime) {
							try {
...
//		}
	...
	public long getInactiveCheckPeriod() {
		return inactiveCheckPeriod;
	}
	public void setInactiveCheckPeriod(long inactiveCheckPeriod) {
		this.inactiveCheckPeriod = inactiveCheckPeriod;
	}
